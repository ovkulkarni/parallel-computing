...
int main(void)
{
    for(int q=0; q < 100; q++){
        ...
        background.g = 0;
        background.b = 0;
        color evens = {100, 100, 0};
        color odds = {0, 100, 100};
        ...
        for( y = 0 ; y < N ; y++ )
        {
            for( x = 0 ; x < M ; x++)
            {
                ...
                int is_even = -1;
                if(intersecting == 0){
                    is_even = ((int)(from.x*14) + (int)(from.z*14)) % 2 == 0;
                }
                if(min_t == max_num){
                    ...
                    if(is_even == -1){
                        ...
                        if(!is_even){
                            if(in_shadow) {
                                rgb[y][x][0] = evens.r*SHADOW;
                                rgb[y][x][1] = evens.g*SHADOW;
                                rgb[y][x][2] = evens.b*SHADOW;
                            }
                            else{
                                rgb[y][x][0] = evens.r*0.5 + evens.r*0.5*lambert;
                                rgb[y][x][1] = evens.g*0.5 + evens.g*0.5*lambert;
                                rgb[y][x][2] = evens.b*0.5 + evens.b*0.5*lambert;
                            }
                        }
                        else {
                            if(in_shadow) {
                                rgb[y][x][0] = odds.r*SHADOW;
                                rgb[y][x][1] = odds.g*SHADOW;
                                rgb[y][x][2] = odds.b*SHADOW;
                            }
                            else{
                                rgb[y][x][0] = odds.r*0.5 + odds.r*0.5*lambert;
                                rgb[y][x][1] = odds.g*0.5 + odds.g*0.5*lambert;
                                rgb[y][x][2] = odds.b*0.5 + odds.b*0.5*lambert;
                            }
                        }
                    }
                }
    ...
    system("convert -loop 0 -delay 1 *video.ppm movie.gif");
    system("rm *video.ppm");
    return 0 ;
}
